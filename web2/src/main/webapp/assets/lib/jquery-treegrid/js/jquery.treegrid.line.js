...
(function ($) {
    ...
    var methods = {
        ...
        initExpander: function () {
            ...
            var isRoot = $this.treegrid('isRoot');
            var isLeaf = $this.treegrid('isLeaf');
            var isFirst = $this.treegrid('isFirst');
            var isLast = $this.treegrid('isLast');
            //var isCollapsed = $this.treegrid('isCollapsed');
            //var isExpanded = $this.treegrid('isExpanded');

            var tpl;
            if (isLast)
                tpl = '<span class="treegrid-expander treegrid-expander-line bottom_line"></span>';
            else
                tpl = '<span class="treegrid-expander treegrid-expander-line center_line"></span>';

            if (isRoot && isFirst && isLast)
                tpl = '<span class="treegrid-expander"></span>';
            else if(isRoot && isFirst && !isLast)
                tpl = '<span class="treegrid-expander treegrid-expander-line root_line"></span>';
        ...
        initIndent: function () {
            ...
            var tpl_none = '<span class="treegrid-indent"></span>';
            var tpl_con_line = '<span class="treegrid-indent treegrid-indent-line con_line"></span>';
            //var tpl_center_line = '<span class="treegrid-indent treegrid-indent-line center_line"></span>';
            //var tpl_bottom_line = '<span class="treegrid-indent treegrid-indent-line bottom_line"></span>';
            var expander = $this.find('.treegrid-expander');


            //console.log('节点：' + expander.parent().text());
            ...
            //var getChildNodes = $this.treegrid('getChildNodes');
            //var hasChildNodes = getChildNodes && getChildNodes.length > 0;
            ...
            var getParentN = function (obj, n) {
                var parent = getParent(obj);
                if (n == 1)
                    return parent;
                else
                    return getParentN(parent, n - 1);
            };

            for (var i = 0; i < depth; i++) {
                if (getParentN($(this), depth - i).treegrid('isLast')) {
                    $(tpl_none).insertBefore(expander);
                } else {
                    $(tpl_con_line).insertBefore(expander);
                }
            }
            return $this;
        },
            ...
            return $(this).each(function () {
                ...
                if (expander) {
                    ...
                    if (!$this.treegrid('isCollapsed')) {
                        if (isRoot === true) {
                            if(isFirst&&isLast){
                                expander.removeClass('treegrid-expander-collapsed one_close');
                                expander.addClass('treegrid-expander-expanded one_open');
                            }
                            else if (isFirst) {
                                expander.removeClass('treegrid-expander-collapsed root_close');
                                expander.addClass('treegrid-expander-expanded root_open');
                            } else if (isLast) {
                                expander.removeClass('treegrid-expander-collapsed bottom_close');
                                expander.addClass('treegrid-expander-expanded bottom_open');
                            }
                            else {
                                expander.removeClass('treegrid-expander-collapsed center_close');
                                expander.addClass('treegrid-expander-expanded center_open');
                            }
                        }
                        else if (isLast) {
                            expander.removeClass('treegrid-expander-collapsed bottom_close');
                        ...
                        if (isRoot === true) {
                            if(isFirst&&isLast){
                                expander.removeClass('treegrid-expander-expanded one_open');
                                expander.addClass('treegrid-expander-collapsed one_close');
                            }
                            else if (isFirst) {
                                expander.removeClass('treegrid-expander-expanded root_open');
                                expander.addClass('treegrid-expander-collapsed root_close');
                            }
                            else if (isLast) {
                                expander.removeClass('treegrid-expander-expanded bottom_open');
                                expander.addClass('treegrid-expander-collapsed bottom_close');
                            }
                            else {
                                expander.removeClass('treegrid-expander-expanded center_open');
                                expander.addClass('treegrid-expander-collapsed center_close');
                            }
                        } else if (isLast) {
                            expander.removeClass('treegrid-expander-expanded bottom_open');
                            expander.addClass('treegrid-expander-collapsed bottom_close');
