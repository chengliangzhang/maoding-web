...
(function () {
    ...
    //获取项目网址，如： http://localhost:8088/ott/
    function getRootPath(){

        //获取当前网址，如： http://localhost:8088/ott/common/widget/ueditor/dialogs/image/image.js
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    //重新定位路径
                    var urlcache = list[i].url;
                    urlcache = urlcache.substring(urlcache.indexOf(listPath),urlcache.length);
                    urlcache = getRootPath() + urlcache;

                    img.setAttribute('src', urlcache + (list[i].url.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                    img.setAttribute('_src', urlcache);
