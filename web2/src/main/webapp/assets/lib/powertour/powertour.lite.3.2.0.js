	...
	Plugin.prototype = {
		...
		**/
		_tourDataVars: function(){
			...
			if(!isNaN(id)){
				...
				scrollSpeed       = (step.scrollSpeed     !== undefined && $.trim(step.scrollSpeed)     != '') ? step.scrollSpeed     : 1000;
				...
				fxIn              = (step.fxIn            !== undefined && $.trim(step.fxIn)            != '') ? step.fxIn            : '';
				fxOut             = (step.fxOut           !== undefined && $.trim(step.fxOut)           != '') ? step.fxOut           : '';				
				showStepDelay     = (step.showStepDelay   !== undefined && $.trim(step.showStepDelay)   != '') ? step.showStepDelay   : 0;			
				keepVisible       = (step.keepVisible     !== undefined && $.trim(step.keepVisible)     != '') ? step.keepVisible     : false;
				...
				**/	
				if(ps !== undefined){
					...
					prevFxOut       = (prevStep.fxOut           !== undefined && $.trim(prevStep.fxOut)           != '') ? prevStep.fxOut           : '';
					keepHighlighted = (prevStep.keepHighlighted !== undefined && $.trim(prevStep.keepHighlighted) != '') ? prevStep.keepHighlighted : false;
				}else{
		...
		**/
		_fxIn: function(csObj, fxIn){ 	
			...
			if(bd.attr(d_pwid) !== undefined){
				...
				if(bd.attr(d_pwfx) == 'false' || fxIn == '' || fxIn == 'none'){
					var show = c_pwsw;
				}else{
		...
		**/
		_endTour: function(type){ 
			...
			**/
			bd.removeAttr(d_pwid+' '+d_pwcs+' '+d_pwps+' '+d_pwrn+' '+d_pwfo);
