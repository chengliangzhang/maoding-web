...
import com.maoding.core.component.fastdfs.common.NameValuePair;
...
public class ProtoCommon {
	...
	public static RecvHeaderInfo recvHeader(InputStream in, byte expect_cmd,
			long expect_body_len) throws IOException {
		byte[] header;
		int bytes;
		long pkg_len;
		...
		if (header[PROTO_HEADER_CMD_INDEX] != expect_cmd) {
			throw new IOException("recv cmd: " + header[PROTO_HEADER_CMD_INDEX]
		...
		if (expect_body_len >= 0 && pkg_len != expect_body_len) {
			throw new IOException("recv body length: " + pkg_len
	...
	public static RecvPackageInfo recvPackage(InputStream in, byte expect_cmd,
		...
		if (totalBytes != header.body_len) {
			throw new IOException("recv package size " + totalBytes + " != "
					+ header.body_len);
		}
	...
	public static NameValuePair[] split_metadata(String meta_buff) {
		return split_metadata(meta_buff, FDFS_RECORD_SEPERATOR,
	...
	public static String pack_metadata(NameValuePair[] meta_list) {
		...
		sb.append(meta_list[0].getName()).append(FDFS_FIELD_SEPERATOR)
				.append(meta_list[0].getValue());
		for (int i = 1; i < meta_list.length; i++) {
			sb.append(FDFS_RECORD_SEPERATOR);
			sb.append(meta_list[i].getName()).append(FDFS_FIELD_SEPERATOR)
	 ...
	 * is alive
	 */
	public static boolean activeTest(InputStream inputStream,
			OutputStream outputStream) throws IOException {
		byte[] header;
		header = packHeader(FDFS_PROTO_CMD_ACTIVE_TEST, 0, (byte) 0);
		outputStream.write(header);
		outputStream.flush();
		RecvHeaderInfo headerInfo = recvHeader(inputStream,
				TRACKER_PROTO_CMD_RESP, 0);
	...
	public static String getToken(String remote_filename, int ts,
			...
			NoSuchAlgorithmException, FastDFSException {
		...
		byte[] buff = new byte[bsFilename.length + bsKey.length
				+ bsTimestamp.length];
		System.arraycopy(bsFilename, 0, buff, 0, bsFilename.length);
		System.arraycopy(bsKey, 0, buff, bsFilename.length, bsKey.length);
		System.arraycopy(bsTimestamp, 0, buff,
				bsFilename.length + bsKey.length, bsTimestamp.length);
	...
	public static String genSlaveFilename(String master_filename,
		...
		if (master_filename.length() < 28 + FDFS_FILE_EXT_NAME_MAX_LEN) {
			throw new FastDFSException("master filename \"" + master_filename
					+ "\" is invalid");
		}

		dotIndex = master_filename.indexOf('.', master_filename.length()
		...
		if (true_ext_name.length() == 0 && prefix_name.equals("-m")) {
			throw new FastDFSException("prefix_name \"" + prefix_name
					+ "\" is invalid");
		}
		...
		if (dotIndex < 0) {
			...
			return master_filename.substring(0, dotIndex) + prefix_name
					+ true_ext_name;
		}
	}
}
