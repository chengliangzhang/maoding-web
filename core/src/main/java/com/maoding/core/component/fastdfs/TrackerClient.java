...
import com.maoding.core.component.fastdfs.conn.pool.TrackerConnectionManager;
...
{
	protected volatile TrackerGroup tracker_group;
	protected volatile byte errno;
...
*/
	...
	*/
	public ConnectionSupplier getConnection() throws IOException
	{
	...
	*/
	public StorageServer getStoreStorage(ConnectionSupplier connectionSupplier) throws IOException
	{
		final String groupName = null;
		return this.getStoreStorage(connectionSupplier, groupName);
	}
	...
	*/
	public StorageServer getStoreStorage(ConnectionSupplier connectionSupplier, String groupName) throws IOException
	{
		byte[] header;
		String ip_addr;
		int port;
		...
		OutputStream out = connectionSupplier.getOutputStream();
		...
		try
		{
			...
			ProtoCommon.RecvPackageInfo pkgInfo = ProtoCommon.recvPackage(connectionSupplier.getInputStream(), 
		...
		finally
		{
			if (bNewConnection)
			{
				TrackerConnectionManager.INSTANCE.returnConnection(connectionSupplier.getInetSocketAddress(),connectionSupplier);
			}
		}
	}
	...
	*/
	public StorageServer[] getStoreStorages(ConnectionSupplier connectionSupplier, String groupName) throws IOException
	{
		byte[] header;
		String ip_addr;
		int port;
	...
	* @return storage server Socket object, return null if fail
	*/
	public StorageServer getFetchStorage(ConnectionSupplier connectionSupplier, 
			String groupName, String filename) throws IOException
	{
		ServerInfo[] servers = this.getStorages(connectionSupplier, ProtoCommon.TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ONE, 
				groupName, filename);
		if (servers == null)
		{
				return null;
	...
	* @return storage server Socket object, return null if fail
	*/
	public StorageServer getUpdateStorage(ConnectionSupplier connectionSupplier, 
			String groupName, String filename) throws IOException
	{
		ServerInfo[] servers = this.getStorages(connectionSupplier, ProtoCommon.TRACKER_PROTO_CMD_SERVICE_QUERY_UPDATE, 
				groupName, filename);
		if (servers == null)
		{
				return null;
	...
	* @return storage servers, return null if fail
	*/
	public ServerInfo[] getFetchStorages(ConnectionSupplier connectionSupplier, 
			String groupName, String filename) throws IOException
	{
		return this.getStorages(connectionSupplier, ProtoCommon.TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ALL, 
				groupName, filename);
	}
	...
	* @return storage server Socket object, return null if fail
	*/
	protected ServerInfo[] getStorages(ConnectionSupplier connectionSupplier, 
	...
	* @return storage server Socket object, return null if fail
	*/
	public StorageServer getFetchStorage1(ConnectionSupplier connectionSupplier, String file_id) throws IOException
	{
		...
		return this.getFetchStorage(connectionSupplier, parts[0], parts[1]);
	}
	...
	* @return storage servers, return null if fail
	*/
	public ServerInfo[] getFetchStorages1(ConnectionSupplier connectionSupplier, String file_id) throws IOException
	{
		String[] parts = new String[2];
		this.errno = StorageClient1.split_file_id(file_id, parts);
		if (this.errno != 0)
		...
		return this.getFetchStorages(connectionSupplier, parts[0], parts[1]);
	}
	...
	*/
	public StructGroupStat[] listGroups(ConnectionSupplier connectionSupplier) throws IOException
	{
		byte[] header;
		boolean bNewConnection;
	...
	*/
	public StructStorageStat[] listStorages(ConnectionSupplier connectionSupplier, String groupName) throws IOException
	{
		final String storageIpAddr = null;
		return this.listStorages(connectionSupplier, groupName, storageIpAddr);
	}
	...
	* @return storage server stat array, return null if fail
	*/
	public StructStorageStat[] listStorages(ConnectionSupplier connectionSupplier, 
	...
	*/
	private boolean deleteStorage(ConnectionSupplier connectionSupplier, 
			String groupName, String storageIpAddr) throws IOException
	{
		byte[] header;
		byte[] bGroupName;
	...
	*/
			...
			String groupName, String storageIpAddr) throws IOException
	{
		...
		{			
			connectionSupplier = trackerGroup.getConnection(serverIndex);
			try
			{
				StructStorageStat[] storageStats = listStorages(connectionSupplier, groupName, storageIpAddr);
		...
		for (serverIndex=0; serverIndex<trackerGroup.tracker_servers.length; serverIndex++)
		{
			...
			try
			{
				if (!this.deleteStorage(connectionSupplier, groupName, storageIpAddr))
				{
					if (this.errno != 0)
					{
						if (this.errno == ProtoCommon.ERR_NO_ENOENT)
