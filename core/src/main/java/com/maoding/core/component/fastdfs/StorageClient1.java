...
*/
	public StorageClient1(ConnectionSupplier connectionSupplier, StorageServer storageServer)
	{
		super(connectionSupplier, storageServer);
	}
  ...
  * @return file id(including group name and filename) if success, <br>
	...
	*/
	       ...
	       String local_filename, String file_ext_name, NameValuePair[] meta_list) throws IOException, FastDFSException
	{
		...
		this.errno = StorageClient1.split_file_id(master_file_id, parts);
		if (this.errno != 0)
		{
			return null;
		}
  ...
  * @return file id(including group name and filename) if success, <br>
	...
	*/
	...
	{
		String[] parts = new String[2];
		this.errno = StorageClient1.split_file_id(appender_file_id, parts);
		if (this.errno != 0)
		{
			return this.errno;
		}
